...
H~0~: **pH does not have an affect on abalone growth**  

H~A~: **pH has an affect on abalone growth**
...
# creating a new object abalone_clean from abalone data set
abalone_clean <- abalone |> 
  # cleaning names to include lowercase and underscore only
  clean_names() |> 
  # selecting only these columns: mean ph and mean change in area
  select(mean_p_h, change_in_area_mm2_d_1_25) |> 
  # creating cleaner column names; "mean_p_h" becomes "mean_ph" and "change_in_area..." becomes "change_in_area"
...
ggplot(data = abalone_clean, # using abalone_clean data frame
       aes(x = mean_ph, # mapping aes, x axis is mean_ph and y is change_in_area
           y = change_in_area)) +
...
**A pair of tanks in which the pH sensor malfunctioned, so there is no pH reading from those tanks.**  
...
**At ph = 8, the model predicts abalone shell growth of 5.36 [5.08, 5.64] mm^2 per day**
